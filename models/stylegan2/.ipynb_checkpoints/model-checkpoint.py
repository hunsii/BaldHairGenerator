...
#         print(style.shape)
#         print(self.modulation(style).shape)
...
#             print("input is not latent")
            styles = [self.style(s) for s in styles]
#         print(f"length of styles: {len(styles)}")
#         for style in styles:
#             print(style.shape)
        if noise is None:
            if randomize_noise:
...
#             print(f"truncation < 1")
            ...
            styles = style_t
        """
        styles = 1xx18x512를 담고 있는 list
        list의 길이는 하나이기에 무조건 들어가며
        ndim은 3이기에 latent = styles[0]
        """
        if len(styles) < 2:
            inject_index = self.n_latent
#             print(styles[0].ndim) # 3ㅇ
            if styles[0].ndim < 3:
